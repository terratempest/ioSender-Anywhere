...

{
    ...
    const string SelectType = "Select edge or corner to probe by clicking on the relevant part of the image above.";

    const string PositionUnknown = "Probing failed, machine position not known.";
    ...
    public EdgeFinderControl() => InitializeComponent();



    public ProbingType ProbingType => ProbingType.EdgeFinderExternal;
...

    {
...

        {
            ...
            GrblUi.ShowError(SelectType, "Edge finder");
    ...
    void AddCorner(ProbingViewModel probing, bool negx, bool negy, double xyClearance)

    {
        ...
        rapidto.Y = probing.StartPosition.Values[GrblConstants.Y_AXIS] - xyClearance * _af[GrblConstants.Y_AXIS];
...

    {
...

        {
...

            {
                ...
                probing.Program.End(PositionUnknown);
            ...
            ok = ok && !_isCancelled && probing.GotoMachinePosition(pos, AxisFlags.Y);

            ok = ok && !_isCancelled && probing.GotoMachinePosition(pos, AxisFlags.X);
            ...
            probing.Program.End(ok ? ProbingCompleted : ProbingFailed);
